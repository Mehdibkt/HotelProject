<?php 
  ...
  if(isset($_GET['fetch_rooms']))
  {
    ...
    while($room_data = mysqli_fetch_assoc($room_res))
    {
      ...
      if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
      {
        $tb_query = "SELECT COUNT(*) AS `total_bookings` FROM `rental`
          WHERE rental_status=? AND room_id=?
          AND check_out > ? AND check_in < ?";

        $values = [1,$room_data['room_id'],$chk_avail['checkin'],$chk_avail['checkout']];
      ...
      {
        if( in_array($fea_row['id'],$feature_list['features']) ){
          $fea_count++;
        }
      ...
      $room_thumb = ROOMS_IMG_PATH."double.jpg";
      ...
      <div class='card mb-4 border-0 shadow' >
        <div class='row g-0 p-3 align-items-center'>
            ...
            <div class='col-md-2 mt-lg-0 mt-md-0 mt-4 text-center '>
              ...
              <a onclick='checkLoginToBook($login,$room_data[room_id])' class='btn btn-sm w-100 text-white custom-bg shadow-none mb-2'>Reserve Now</a>
            </div>
        </div>
      </div>
      ";
